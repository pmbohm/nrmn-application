...
public class IngestionController {
    ...
    public ResponseEntity ingest(@PathVariable("job_id") Long jobId) {
        ...
        try {
            ...
            Validated<StagedRowError, Seq<Tuple2<String, Object>>> v = validation.validate(rows.get(0));
            rows.stream()
                    .map(validation::preValidated)
                    .filter(Optional::isPresent).map(Optional::get)
                    .forEach(surveyIngestionService::ingestStagedRow);
